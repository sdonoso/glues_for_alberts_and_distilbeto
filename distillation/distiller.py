...
class Distiller:
    def __init__(
        ...
        checkpoint = torch.load(
            '/home/sedonoso/results_distilbert/checkpoint.pth')
        self.params = params
        ...
        if params.group_by_size:
            ...
            sampler = GroupedBatchSampler(sampler=sampler, group_ids=groups,
        ...
        optimizer_grouped_parameters = [
            {
                "params": [
                    p for n, p in student.named_parameters() if
        ...
        logger.info(
            ...
            % sum([p.numel() for p in self.student.parameters() if
                   p.requires_grad])
        )
        logger.info("------ Number of parameters (student): %i" % sum(
            [p.numel() for p in self.student.parameters()]))
        self.optimizer = AdamW(
            optimizer_grouped_parameters, lr=params.learning_rate,
            eps=params.adam_epsilon, betas=(0.9, 0.98)
        )
    ...
    def prepare_batch_mlm(self, batch):
        ...
        tgt_ids = torch.multinomial(x_prob / x_prob.sum(), n_tgt,
                                    replacement=False)
        ...
        probs = torch.multinomial(self.pred_probs, len(_token_ids_real),
                                  replacement=True)
        _token_ids = (
    ...
    def round_batch(self, x: torch.tensor, lengths: torch.tensor):
        ...
        if ml1 % 8 != 0:
            ...
            padding_tensor = torch.zeros(bs2, pad, dtype=torch.long,
    ...
    def train(self):
        ...
        for _ in range(self.params.n_epoch):
            ...
            for batch in iter_bar:
                if self.n_iter < 4318000 and self.epoch == 0:
                    self.n_iter += 1
                    self.n_total_iter += 1
                    continue
    ...
    def step(self, input_ids: torch.tensor, attention_mask: torch.tensor,
        ...
        if self.fp16:
            with autocast():
                if self.mlm:
                    s_forward = self.student(
                        input_ids=input_ids, attention_mask=attention_mask
                    )  # (bs, seq_length, voc_size)
                    with torch.no_grad():
                        t_forward = self.teacher(
                            input_ids=input_ids, attention_mask=attention_mask
                        )  # (bs, seq_length, voc_size)
                else:
                    s_forward = self.student(
                ...
                s_logits = s_forward.logits
                t_logits = t_forward.logits
                s_hidden_states = s_forward.hidden_states
                t_hidden_states = t_forward.hidden_states
                ...
                if self.alpha_mse > 0.0:
                    loss_mse = self.mse_loss_fct(s_logits_slct,
                                                 t_logits_slct) / s_logits_slct.size(
                        0
                ...
                if self.alpha_cos > 0.0:
                    ...
                    s_hidden_states_slct = torch.masked_select(s_hidden_states,
                                                               mask)  # (bs * seq_length * dim)
                    s_hidden_states_slct = s_hidden_states_slct.view(-1,
                                                                     dim)  # (bs * seq_length, dim)
                    t_hidden_states_slct = torch.masked_select(t_hidden_states,
                                                               mask)  # (bs * seq_length * dim)
                    t_hidden_states_slct = t_hidden_states_slct.view(-1,
                                                                     dim)  # (bs * seq_length, dim)

                    target = s_hidden_states_slct.new(
                        s_hidden_states_slct.size(0)).fill_(
                        1)  # (bs * seq_length,)
                    loss_cos = self.cosine_loss_fct(s_hidden_states_slct,
                                                    t_hidden_states_slct,
                                                    target)
            ...
            loss_ce = (
                    self.ce_loss_fct(
                        nn.functional.log_softmax(
                            s_logits_slct / self.temperature, dim=-1),
                        nn.functional.softmax(t_logits_slct / self.temperature,
                                              dim=-1),
                    )
                    * (self.temperature) ** 2
            )
        ...
        del t_logits_slct
        del loss
        del s_hidden_states_slct
        del t_hidden_states_slct
        del target
    ...
    def log_tensorboard(self):
        ...
        for param_name, param in self.student.named_parameters():
            self.tensorboard.add_scalar(
                ...
                scalar_value=param.data.mean(), global_step=self.n_total_iter
            )
            self.tensorboard.add_scalar(
                tag="parameter_std/" + param_name,
                scalar_value=param.data.std(), global_step=self.n_total_iter
            )
        ...
        self.tensorboard.add_scalar(tag="losses/loss",
                                    ...
                                    global_step=self.n_total_iter)
        self.tensorboard.add_scalar(
            tag="losses/loss_ce", scalar_value=self.last_loss_ce,
            global_step=self.n_total_iter
        )
        if self.alpha_mlm > 0.0:
            self.tensorboard.add_scalar(
                tag="losses/loss_mlm", scalar_value=self.last_loss_mlm,
                global_step=self.n_total_iter
            )
        if self.alpha_clm > 0.0:
            self.tensorboard.add_scalar(
                tag="losses/loss_clm", scalar_value=self.last_loss_clm,
                global_step=self.n_total_iter
            )
        if self.alpha_mse > 0.0:
            self.tensorboard.add_scalar(
                tag="losses/loss_mse", scalar_value=self.last_loss_mse,
                global_step=self.n_total_iter
            )
        if self.alpha_cos > 0.0:
            self.tensorboard.add_scalar(
                tag="losses/loss_cos", scalar_value=self.last_loss_cos,
                global_step=self.n_total_iter
            )
        self.tensorboard.add_scalar(
            tag="learning_rate/lr", scalar_value=self.scheduler.get_lr()[0],
            global_step=self.n_total_iter
        )
        ...
        self.tensorboard.add_scalar(
            tag="global/speed", scalar_value=time.time() - self.last_log,
            global_step=self.n_total_iter
        )
    ...
    def save_checkpoint(self, checkpoint_name: str = "checkpoint.pth",
        ...
        if not self.is_master:
            return
        mdl_to_save = self.student.module if hasattr(self.student,
                                                     "module") else self.student
        mdl_to_save.config.save_pretrained(self.dump_path)
        if save_last:
            state_dict = mdl_to_save.state_dict()
            torch.save(state_dict,
                       os.path.join(self.dump_path, checkpoint_name))
        else:
            checkpoint = {"model": mdl_to_save.state_dict(),
                          "optimizer": self.optimizer.state_dict(),
                          "scaler": self.scaler.state_dict(),
                          "iter": self.n_iter,
                          "total_iter": self.n_total_iter}
