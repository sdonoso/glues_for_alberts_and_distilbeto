...
class LmSeqsDataset(Dataset):
    ...
    def load_data(self):
        logger.info(f"Loading data from {self.params.data_file}")
        all_data = []
        all_lengths = []
        with open(self.params.data_file, "rb") as fp:
            count = 0
            while True:
                try:
                    count += 1
                    logger.info(f"starting with {count}")
                    self.token_ids = np.array(pickle.load(fp),dtype=object)
                    self.lengths = np.array([len(t) for t in self.token_ids])
                    self.check()
                    self.remove_long_sequences()
                    self.remove_empty_sequences()
                    self.remove_unknown_sequences()
                    self.check()
                    all_data.extend(self.token_ids)
                    all_lengths.extend(self.lengths)
                    logger.info(f"finished {count}")
                except EOFError:
                    break
        self.token_ids = np.array(all_data,dtype=object)
        self.lengths = np.array(all_lengths,dtype=object)
    ...
    def check(self):
        ...
        assert all(self.lengths[i] == len(self.token_ids[i]) for i in
    ...
    def remove_long_sequences(self):
        ...
        new_lengths = []
        if self.params.mlm:
            cls_id, sep_id = self.params.special_tok_ids["cls_token"], \
                             self.params.special_tok_ids["sep_token"]
        else:
            cls_id, sep_id = self.params.special_tok_ids["bos_token"], \
    ...
    def batch_sequences(self, batch):
        ...
        tk_ = [list(t.astype(int)) + [pad_idx] * (max_seq_len_ - len(t)) for t
               in token_ids]
